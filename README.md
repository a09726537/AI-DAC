...
- configuration information describing the documented laboratory environment.

The objective is to make the reported AI-DAC results transparent, traceable, repeatable, and auditable within the boundaries of the documented experimental configuration.
...
The current AI-DAC evaluation does **not** use confidential, proprietary, or operational security data obtained from companies, government institutions, or public authorities.

The experiments are based exclusively on the following data sources:
...
2. **Publicly available benchmark datasets**  
   Established cybersecurity datasets used for external benchmarking and comparative validation, including UNSW-NB15 and NSL-KDD.
...
No production database records, confidential organizational logs, government security records, or personal customer data are included in the reproducibility package.

The experimental data were not obtained through access granted by a company, government institution, or the University of Vienna. The University affiliation identifies the academic context of the dissertation and does not indicate that the University supplied the experimental data.
...
The primary evaluation is based on a controlled SQL dataset generated within the AI-DAC laboratory environment.

Public cybersecurity datasets are used as external benchmarks to evaluate whether selected detection components remain effective beyond the primary controlled SQL dataset.

Results obtained from public network-intrusion datasets must therefore be interpreted as supplementary benchmark evidence rather than as direct substitutes for relational database telemetry.

All reported findings are bounded by:
...
| Full AI-DAC model F1-score | 0.950 |
...
The complete metric definitions, source files, rounding conventions, and reproduction statuses are provided in the final audit report and the machine-readable audit summaries.
...
| `final_reproducibility_audit_report.txt` | Human-readable final reproducibility audit report |
...
| `CITATION.cff` | Citation metadata for the AI-DAC reproducibility package |
...
After downloading or cloning the repository, verify the integrity of the reproducibility archive by running:
